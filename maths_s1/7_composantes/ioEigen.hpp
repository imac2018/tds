 ...
 * Anti-doublon
 */
#ifndef __EIGEN_IO_HPP__
#define __EIGEN_IO_HPP__
...
#include <Eigen/Dense>
  ...
  void readMatrixFromHeader(EigenMatrix &M,
    ...
    // if the matrix is already built
    if(M.rows() != row || M.cols() != column)
      M.resize(row,column);
	std::string strLine;
	unsigned int i=0;
	unsigned int j=0;
	// From the matrixFile parse the value and store in a matrix, and read the matrix
	while(std::getline(matrixFile,strLine))
	{
		std::stringstream lineStr(strLine);
		std::string val;
		while(std::getline(lineStr,val,'\t')){
			//std::cout << 
			//std::cout<<"str = "<<val<<std::endl;
			if(j>0)
				std::istringstream(val) >> M(i,j-1); // The first column of the csv is ignored therefore 
			j++;
		}
		j=0;
		i++;
  ...
  void saveMatrix(const EigenMatrix &M,
    ...
    for (int i=0; i<M.rows(); ++i){
      for (int j=0; j<M.cols(); ++j)
        if(commaMode) matrixFile << M(i,j) << ", ";
        else matrixFile << M(i,j) << " ";
