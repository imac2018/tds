...
int main(int argc, char** argv) {
  ...
  SDLWindowManager windowManager(800, 600, "Ldussouc - GLImac");
    ...
    std::string shaderFS(argv[1]);
    std::string shaderVS(argv[2]);

    FilePath applicationPath(argv[0]);
    Program program = loadProgram(applicationPath.dirPath() + "shaders/" + shaderFS,
                                  applicationPath.dirPath() + "shaders/" + shaderVS);
    program.use();
